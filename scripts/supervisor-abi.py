...
class Supervisor:
	...
	# rescue command receiver # we can publish a string to /rescue_cmd, then inside the callback, we can create the object list and set the state to rescue state
        ...
        self.trans_listener.waitForTransform(self.origin_frame, '/base_link', rospy.Time(), rospy.Duration(5))
    ...
    def add_object_to_dict(self, dist, cl, obj_loc):
        ...
        if cl in self.object_db.keys():
            if abs(dist-self.object_db[cl][0])>0.01 and (dist<self.object_db[cl][0]):
    ...
    def obj_detected_callback_wo_tf(self,  msg):
        ...
        theta_c = thetaright + (thetaleft - thetaright) / 2
        x_c = d * np.cos(theta_c) 
        y_c = d * np.sin(theta_c)
        z_c = 0
        x_c_t = np.array([x_c, y_c, z_c, 1])
        
        print(f'robot @ x:{self.x}, y:{self.y}, theta:{self.theta}')
        
        #Conversion from robot frame to map frame. 
        #Note that since camera frame and robot frame and very close together, we assume the camera
        #and robot frame to be the same
        #Angle of the object in robot frame = theta_c(angle between heading and object) + theta(heading)
        #Transformation from robot frame to map frame
        
        theta_z = self.theta + theta_c
        
        T_MR = np.array([
        [np.cos(theta_z), -np.sin(theta_z), 0, self.x], 
        [np.sin(theta_z),  np.cos(theta_z), 0, self.y], 
        [0,                              0, 1,      0], 
        [0,                              0, 0,      1],
        ])
        ...
        print(f'obj in map co-ordinates: {x_m}')
    ...
    def has_idled(self):
        return self.mode == Mode.IDLE and \
               rospy.get_rostime() - self.idle_start > rospy.Duration.from_sec(self.idle_time)
    ...
    def run(self):
        ...
        self.load_wpts("./savewaypts.txt") if self.use_waypt_flg else print("use rviz") 
